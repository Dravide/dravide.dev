...
@section('content')
    <div class="min-h-screen bg-white text-black font-mono selection:bg-black selection:text-white pb-20" x-data="{ 
                selectedProject: null, 
                selectedPost: null,
                currentImageIndex: 0,
                nextImage() {
                    if (!this.selectedProject) return;
                    let total = 1 + (this.selectedProject.images ? this.selectedProject.images.length : 0);
            ...
            <section class="mb-16">
                @if($profile && $profile->avatar)
                    <div class="mb-6">
                        <img src="{{ Storage::url($profile->avatar) }}" alt="{{ $profile->name }}"
                            class="w-16 h-16 rounded-full object-cover grayscale hover:grayscale-0 transition-all duration-500">
                    </div>
                @endif

                <h1 class="text-2xl font-bold mb-6">
                    Hey, I'm <span
                        class="text-accent underline decoration-2 underline-offset-4">{{ $profile->name ?? 'Admin' }}</span>!
                </h1>
                ...
                <div class="flex items-center gap-4 mb-8">
                    <a href="https://wa.me/{{ $profile->whatsapp_number ?? '#' }}" target="_blank"
                        class="bg-black text-white text-xs font-bold px-6 py-2.5 rounded hover:bg-gray-800 transition-colors">
                        Message on WhatsApp
                    </a>
                    <a href="mailto:{{ $profile->email ?? '#' }}"
                        class="border border-gray-200 text-black text-xs font-bold px-6 py-2.5 rounded hover:bg-gray-50 transition-colors">
                        Send an email
                    </a>
                </div>
            ...
            <section class="mb-16">
                ...
                <div class="flex flex-wrap gap-2">
                    @forelse($techStacks as $tech)
                        <div
                            class="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-lg text-xs font-medium text-gray-700 hover:border-gray-300 transition-all cursor-default">
                            @if($tech->icon)
                        ...
                        <div class="group bg-white border border-gray-100 p-5 rounded-xl hover:border-gray-200 hover:shadow-sm transition-all relative overflow-hidden cursor-pointer"
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 group-hover:text-black transition-colors">
                                ...
                                @if($portfolio->is_visible)
                                    <span
                                        class="text-[9px] font-bold px-2 py-0.5 bg-green-50 text-green-600 rounded-full tracking-wide">Active</span>
                                @endif
                            </div>
                            ...
                            @if($portfolio->techStacks->isNotEmpty())
                                <div class="mt-4 flex flex-wrap gap-1.5">
                                    @foreach($portfolio->techStacks->take(5) as $tech)
                                        <div class="w-5 h-5 rounded-md bg-gray-50 flex items-center justify-center text-[10px]"
                                            title="{{ $tech->name }}">
                                            <i class="{{ $tech->icon }}" style="color: {{ $tech->color ?? '#9ca3af' }}"></i>
                                        </div>
                                    @endforeach
                                    @if($portfolio->techStacks->count() > 5)
                                        <span
                                            class="text-[8px] text-gray-400 font-bold">+{{ $portfolio->techStacks->count() - 5 }}</span>
                                    @endif
                                </div>
                            @endif
                            ...
                            <div class="mt-4 flex items-center text-[10px] font-bold text-accent uppercase tracking-wider group-hover:translate-x-1 transition-transform"
                                @click.stop="selectedProject = {{ $portfolio->toJson() }}; currentImageIndex = 0">
                ...
                <div class="flex flex-wrap gap-4">
                    @if($profile->github_url)
                        <a href="{{ $profile->github_url }}" target="_blank"
                            class="flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-black transition-colors">
                    ...
                    @forelse($blogPosts as $post)
                        <article @click="selectedPost = {{ json_encode($post) }}" class="cursor-pointer group">
                            <div class="flex items-baseline justify-between gap-4">
                                <h3 class="text-sm font-bold text-black group-hover:text-accent transition-colors">
                                    {{ $post->title }}
                                </h3>
                ...
                <div class="space-y-2">
                    @if($profile->email)
                        <p class="text-xs text-gray-600">
                            You can reach me anytime at <a href="mailto:{{ $profile->email }}"
                                class="text-black font-bold border-b border-black/10 hover:border-black/40 transition-colors">{{ $profile->email }}</a>
                        </p>
                    @endif
                    <p class="text-xs text-gray-600">
                        Or book a call on <a href="#"
                            class="text-black font-bold border-b border-black/10 hover:border-black/40 transition-colors">Cal.com</a>
                    </p>
                </div>
            </section>
        </main>
        ...
        <footer class="container-custom px-6 border-t border-gray-50 pt-8 flex items-center justify-between">
            ...
            <div class="text-[10px] text-gray-400">
                This website is <a href="#"
                    class="text-black/60 font-medium hover:text-black transition-colors underline underline-offset-4 decoration-black/10 hover:decoration-black/40">open-source</a>
            </div>
        </footer>
        ...
        <template x-teleport="body">
            <div x-show="selectedProject || selectedPost"
                ...
                <!-- Backdrop -->
                <div x-show="selectedProject || selectedPost" x-transition:enter="ease-out duration-300"
                ...
                <template x-if="selectedProject">
                    <div
                        ...
                        <div class="relative h-64 sm:h-80 bg-gray-900 flex-shrink-0 group/slider overflow-hidden"
                            style="min-height: 320px;">
                            <!-- Slider Items -->
                            <div class="relative w-full h-full">
                                <!-- Main Image -->
                                <div x-show="currentImageIndex === 0" x-transition:enter="transition ease-out duration-700"
                                    x-transition:enter-start="opacity-0 scale-110"
                                    x-transition:enter-end="opacity-100 scale-100"
                                    x-transition:leave="transition ease-in duration-500"
                                    x-transition:leave-start="opacity-100 scale-100"
                                    x-transition:leave-end="opacity-0 scale-110" class="absolute inset-0 z-0">
                                    <template x-if="selectedProject.image">
                                        <img :src="'/storage/' + selectedProject.image" :alt="selectedProject.title"
                                            class="w-full h-full object-cover">
                                    </template>
                                    <template x-if="!selectedProject.image">
                                        <div
                                            class="w-full h-full flex items-center justify-center bg-gray-800 text-gray-600">
                                            <i :class="selectedProject.category_icon || 'ti ti-photo-off'"
                                                class="text-6xl"></i>
                                        </div>
                                    </template>
                                </div>
                                ...
                                <!-- Gallery Images -->
                                <template x-if="selectedProject.images">
                                    <template x-for="(img, index) in selectedProject.images" :key="img.id">
                                        <div x-show="currentImageIndex === index + 1"
                            ...
                            <template x-if="selectedProject.images && selectedProject.images.length > 0">
                                <div
                                    class="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover/slider:opacity-100 transition-opacity duration-300">
                                    <button @click="prevImage"
                                        class="w-10 h-10 rounded-xl bg-black/40 backdrop-blur-md text-white flex items-center justify-center hover:bg-black/60 transition-all transform hover:-translate-x-1">
                        ...
                        <div class="p-6 sm:p-8 overflow-y-auto">
                            <div class="flex flex-col gap-2 mb-4">
                                <template x-if="selectedProject.category">
                                    <div class="mb-2">
                                ...
                                <h2 x-text="selectedProject.title" class="text-xl font-bold leading-tight"></h2>
                            </div>
                            <div class="prose prose-sm max-w-none text-gray-600 mb-8 leading-relaxed">
                                <p x-text="selectedProject.description"></p>
                            </div>

                            <template x-if="selectedProject.tech_stacks && selectedProject.tech_stacks.length > 0">
                                <div class="mb-8">
                                    <h4 class="text-[10px] font-bold tracking-[0.2em] text-gray-400 uppercase mb-3">Built
                                        with</h4>
                                    <div class="flex flex-wrap gap-2">
                                        <template x-for="tech in selectedProject.tech_stacks" :key="tech.id">
                                            <div
                                                class="flex items-center gap-1.5 px-2 py-1 bg-gray-50 border border-gray-100 rounded text-[10px] font-bold text-gray-600">
                                                <i :class="tech.icon" :style="'color: ' + (tech.color || '#9ca3af')"></i>
                ...
                <template x-if="selectedPost">
                    <div
                        ...
                        <div x-show="selectedPost.image" class="relative h-48 sm:h-64 bg-gray-50 flex-shrink-0">
                            <img :src="'/storage/' + selectedPost.image" :alt="selectedPost.title"
                                class="w-full h-full object-cover">
                        </div>
                        <div class="p-6 sm:p-8 overflow-y-auto">
                            <div class="mb-6">
                                <span
                                    x-text="selectedPost.published_at ? new Date(selectedPost.published_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : new Date(selectedPost.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })"
                                    class="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-2"></span>
                                <h2 x-text="selectedPost.title" class="text-xl font-bold text-black leading-tight"></h2>
                            </div>
                            <div class="prose prose-sm max-w-none text-gray-600 leading-relaxed whitespace-pre-wrap font-sans"
